...
namespace TrackDb.Lib.Logging
{
		...
		: ContentBase<TransactionContent>
	{
			...
			IImmutableDictionary<string, TableSchema> tableSchemaMap)
		{
			...
			var tombstoneBlock = transactionLog.TableBlockBuilderMap.ContainsKey(tombstoneSchema.TableName)
				? transactionLog.TableBlockBuilderMap[tombstoneSchema.TableName]
				: null;

			foreach (var tableName in tableSchemaMap.Keys)
			{
				var newRecordsContent = transactionLog.TableBlockBuilderMap.ContainsKey(tableName)
					? transactionLog.TableBlockBuilderMap[tableName].ToLog()
					: null;
			...
			foreach (var pair in Tables)
			{
				...
				if (tableTransactionContent.TombstoneRecordIds != null)
				{
					...
					var tombstoneRecords = tableTransactionContent.TombstoneRecordIds
						.Select(id => new TombstoneRecord(id, null, tableName, DateTime.Now));
